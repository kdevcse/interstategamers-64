#include <nusys.h>
#include "../../config.h"
#include "../sprites/right_arrow_16.h"
...
static void DrawArrow(int x, int y, float scale_x, float scale_y);
...
void stage00_draw(void)
{
	...
	if (scene == -1) {
		...
		sprintf(info, "Stage: %d\npos_y: %d", current_stage, pos_y);
	...
	switch(scene){
		...
		case 0:
			...
			DrawText(MENU_OPTIONS_TXT_POS_X, MENU_OPTIONS_TXT_POS_Y, "OPTIONS", 
				TEXT_HALIGN_LEFT, TEXT_VALIGN_TOP);
			DrawArrow(ICON_DEFAULT_X_POS, pos_y, 0.5, 0.5);
			break;
		default:
			ClearBackground(DEF_BG_R, DEF_BG_G, DEF_BG_B);
			DrawBigImg32(TITLE_X_POS, TITLE_Y_POS, 200, 66, IGMainWhite200);
